...
SERVICE_URL = os.environ.get('RENDER_EXTERNAL_URL', f'https://{SERVICE_NAME}.onrender.com')
...
# ========== ОБРАБОТЧИКИ СООБЩЕНИЙ (ОСТАВИТЬ БЕЗ ИЗМЕНЕНИЙ) ==========
...
def home():
    ...
    <html>
        ...
        <body style="text-align:center;padding:50px;font-family:Arial;">
            ...
            <p>Keep-alive активен - Render не заснет</p>
            <p style="margin-top:50px;">Последний пинг: {datetime.datetime.now().strftime("%H:%M:%S")}</p>
        </body>
    </html>
    """
...
def ping():
    ...
    return {'status': 'alive', 'time': datetime.datetime.now().isoformat()}, 200

# ========== НАСТРОЙКА ВЕБ-ХУКА ==========
def setup_webhook():
    """Настраивает веб-хук при запуске"""
    try:
        # Удаляем старый веб-хук (чтобы избежать 409)
        bot.remove_webhook()
        time.sleep(1)
...
if __name__ == '__main__':
    logger.info("🚀 Запуск бота с защитой от зависания...")
